...
const NewTaskDialog = ({ boards }: Props) => {
  ...
  const formSchema = z.object({
    ...
    board: z.string().min(1), // simplified: only need a non‑empty value
  ...
  // Auto‑select board when there is exactly one
  useEffect(() => {
    if (boards && boards.length === 1) {
      form.setValue("board", boards[0].id);
    }
  }, [boards, form]);
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <div className="space-y-4">
                      ...
                      <FormItem>
                        ...
                        >
                          <FormControl>
                            <SelectTrigger>
                              <SelectValue
                                placeholder={c("priorityPlaceholder")}
                              />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                ...
                />
                {/* Show board selector only if there are multiple projects */}
                {boards.length > 1 && (
                  <FormField
                    control={form.control}
                    name="board"
